...
callservice(){
  ...
  do
    ...
    output=`aws lambda invoke --invocation-type RequestResponse --function-name test7FromCLI --region us-east-1 --payload "{"\"uuid\"":"\"$s3uuid\""}" /dev/stdout | head -n 1 | head -c -2 ; echo` 
    # grab end time
    #time2=200
    ...
    x=`aws s3 cp s3://uwt-prms/$s3uuid/out $path/$s3uuid/output --recursive`
    #delete files on S3
    x=`aws s3 rm s3://uwt-prms/$s3uuid --recursive`
    ...
    echo "$i,$threadid,$cont,$cputype,$cpusteal,$vuptime,$pid,$cpuusr,$cpukrn,$elapsedtime,$newcont"
